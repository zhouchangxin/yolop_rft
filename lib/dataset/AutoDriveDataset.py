...
class AutoDriveDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if det_label.size > 0:
            ...
            labels[:, 1] = ratio[0] * w * \
                (det_label[:, 1] - det_label[:, 3] / 2) + pad[0]  # pad width
            labels[:, 2] = ratio[1] * h * \
                (det_label[:, 2] - det_label[:, 4] / 2) + pad[1]  # pad height
            labels[:, 3] = ratio[0] * w * \
                (det_label[:, 1] + det_label[:, 3] / 2) + pad[0]
            labels[:, 4] = ratio[1] * h * \
                (det_label[:, 2] + det_label[:, 4] / 2) + pad[1]
        ...
        if self.cfg.num_seg_class == 3:
            _, seg0 = cv2.threshold(
                seg_label[:, :, 0], 128, 255, cv2.THRESH_BINARY)
            _, seg1 = cv2.threshold(
                seg_label[:, :, 1], 1, 255, cv2.THRESH_BINARY)
            _, seg2 = cv2.threshold(
                seg_label[:, :, 2], 1, 255, cv2.THRESH_BINARY)
        else:
