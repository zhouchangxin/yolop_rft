...
def show_seg_result(img, result, index, epoch, save_dir=None, is_ll=False, palette=None, is_demo=False, is_gt=False):
    ...
    if not is_demo:
        ...
        color_area[result[0] == 1] = [0, 0, 255]
        color_area[result[1] == 1] = [0, 255, 0]
        # 画中间线
        mid_line = []
        for i in range(result[0].shape[1]):
            for j in range(result[0].shape[0]):
                if j != result[0].shape[0]-1:
                    if (result[0][i][j] == 0) and (result[0][i][j+1] == 1):
                        mid_line.append(j)
                    elif (result[0][i][j] == 1) and (result[0][i][j+1] == 0):
                        mid_line.append(j)
                    # else:
                     #   if j+1 == result[0].shape[0]:
                       #     mid_line.append(0)
                       #     mid_line.append(result[0].shape[1])
                else:
                    mid_line.append(0)
                    mid_line.append(result[0].shape[1])
            print("****************", mid_line)
            mid_line_num = (mid_line[0]+mid_line[1])//2
            result[0][i][mid_line_num] == 2
        color_area[result[0] == 2] = [255, 0, 0]
